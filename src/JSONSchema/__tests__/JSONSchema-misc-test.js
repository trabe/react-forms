...
describe('JSONSchema custom test cases', function() {
  ...
  it('custom format function with custom error info', function() {
    let validate = validator({
      type: 'object',
      format(value) {
        if (value.foo !== value.bar) {
          return {field: 'foo', message: 'foo should be equal to bar', custom: 'some value'};
        } else {
          return true;
        }
      },
    ...
    assert(validate.errors[0].field === 'data.foo');
    assert(validate.errors[0].message === 'foo should be equal to bar');
    assert(validate.errors[0].custom === 'some value');
  })
